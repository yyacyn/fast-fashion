...
export function StoreMap({ stores, selectedStore, setSelectedStore }: StoreMapProps) {
            ...
            {stores.features.map((store: any) => {
                ...
                return (
                    ...
                    >
                        <Popup>
                            <div>
                                <h3 className="font-semibold text-lg">{store.properties.name}</h3>
                                <p className="text-sm">{store.properties.address}</p>
                                <button
                                    className="text-sm text-blue-600 hover:underline mt-1"
                                    onClick={() => {
                                        if (store.properties.web) {
                                            window.open(store.properties.web, "_blank", "noopener,noreferrer")
                                        }
                                    }}
                                >
                                    View Details
